...
function displayForm() {
  ...
  for (var i = 0; i < prooduct_container.length; i++) {
    cartoona += `
        <tr>
        <td>${i}</td>
        <td>${prooduct_container[i].ptitle}</td>
        <td>${prooduct_container[i].pprice}</td>
        <td>${prooduct_container[i].pptax}</td>
        <td>${prooduct_container[i].pads}</td>
        <td>${prooduct_container[i].pdiscount}</td>
        <td>${prooduct_container[i].ptotal}</td>
        <td>${prooduct_container[i].pcate}</td>
...
function searchProduct(term)
{
    if (searchmood=='title')
    {
        ...
        for(var i=0;i<prooduct_container.length;i++)
        {
            ...
            <tr>
            <td>${i}</td>
            <td>${prooduct_container[i].ptitle.toLowerCase().replace(term,`<span class="text-danger fw-bold">${term}</span>`)}</td>
            <td>${prooduct_container[i].pprice}</td>
            <td>${prooduct_container[i].pptax}</td>
            <td>${prooduct_container[i].pads}</td>
            <td>${prooduct_container[i].pdiscount}</td>
    ...
    else
    {
        ...
        for(var i=0;i<prooduct_container.length;i++)
        {
            ...
            <td>${prooduct_container[i].pdiscount}</td>
            <td>${prooduct_container[i].ptotal}</td>
            <td>${prooduct_container[i].pcate.toLowerCase().replace(term,`<span class="text-danger fw-bold">${term}</span>`)}</td>
            <td><button class="btn btn-outline-danger" onclick="Delete(${i})">Delete</button></td>
            <td><button class="btn btn-outline-success" onclick="update(${i})">Update</button></td>
            </tr>
            `
